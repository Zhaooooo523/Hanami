...
```text
https://zhaooooo523.github.io/Hanami/#amount=850&merchant=%E5%85%A8%E8%81%AF&last4=1234&category=%E9%A4%90%E9%A3%B2&date=2026-08-03
```

支援參數為 `amount`、`merchant`、`last4`、`category`、`date`；也可使用中文鍵名 `金額`、`商家`、`卡片末四碼`、`分類`、`日期`。日期格式為 `YYYY-MM-DD`，分類需為「餐飲、交通、購物、生活、娛樂、醫療、其他」之一。

花見會解析 hash、開啟預填表單，接著立即清除網址中的暫存資料。資料不會自動寫入，只有在使用者檢查內容並按下「確認並儲存」後才會加入帳本。
